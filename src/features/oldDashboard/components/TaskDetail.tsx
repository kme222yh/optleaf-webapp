...
const areTaskDataDifferent = (
    t1: UpdateTaskMutationVariables | undefined,
    t2: UpdateTaskMutationVariables | undefined
) => t1?.name !== t2?.name || t1?.description !== t2?.description;
...
export function TaskDetail() {
    ...
    if (
        areTaskDataDifferent(
            TaskInfoStatus.oldData,
            query.data?.task as UpdateTaskMutationVariables
        )
    ) {
        TaskInfoStatus.oldData = query.data
            ?.task as UpdateTaskMutationVariables;
    ...
    return (
        ...
        >
            <form
                className="TaskDetail-form"
                onChange={taskInfoForm.handleSubmit(taskInfoIsValid)}
            >
                <input
                    className="TaskDetail-title"
                    {...taskInfoForm.register('name', {
                        required: '名前を入力してください'
                    })}
                />
                <div className="TaskDetail-row">
                    <p className="TaskDetail-row-head">Created</p>
                    <p className="TaskDetail-row-body">
                        {query.data?.task?.created_at}
                    </p>
                </div>
                {query.data?.task?.due_date ? (
                    <div className="taskInfo-row">
                ...
                <textarea
                    className="TaskDetail-description"
                    {...taskInfoForm.register('description', {
            ...
            <button
                type="button"
                className="TaskDetail-deleteButton"
                onClick={deleteTask}
            >
