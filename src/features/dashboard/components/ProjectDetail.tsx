...
import { useProjectQuery, useUpdateProjectMutation, UpdateProjectMutationVariables, useDeleteProjectMutation } from '@/graphql/generated'
...
export function ProjectDetail() {
    ...
    return (
        <InfoBlock className='ProjectDetail' isLoading={query.isLoading || mutation.isLoading || deleter.isLoading}>
            <form className="ProjectDetail-form" onFocus={clearProjectUpdateTimeout} onBlur={projectInfoForm.handleSubmit(projectInfoIsValid, projectInfoIsInValid)}>
                <input className="ProjectDetail-title" {...projectInfoForm.register('name', { required: '名前を入力してください' })} />
                <div className='ProjectDetail-row'>
                    <p className='ProjectDetail-row-head'>Created</p>
                    <p className='ProjectDetail-row-body'>{query.data?.project?.created_at}</p>
                </div>
                <div className='ProjectDetail-row'>
                    <p className='ProjectDetail-row-head'>Owner</p>
                    <p className='ProjectDetail-row-body'>{query.data?.project?.owner?.name}</p>
                </div>
                <textarea className="ProjectDetail-description" {...projectInfoForm.register('description', { required: '詳細を入力してください' })} />
            </form>

            <button type='button' className='ProjectDetail-deleteButton' onClick={deleteProject}>
